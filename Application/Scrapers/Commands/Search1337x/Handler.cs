...
{
    public class Handler : IRequestHandler<Command, Result<List<X1337Preview>>>
    {
        ...
        public async Task<Result<List<X1337Preview>>> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Error occurred while handling Search1337x command");
